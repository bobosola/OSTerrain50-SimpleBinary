...
/*
   ...
   as pairs of digits, or pairs of digits preceded by a two-character grid ref. See:
   https://en.wikipedia.org/wiki/Ordnance_Survey_National_Grid for grid details.

   Selsey Bill: 485669,092167 or SZ 85669 92167
   Cape Wrath:  225638,974936 or NC 25638 74936
   ...
   • may sometimes omit the spaces (SZ8554592142)
   • should have the same number of digits in the eastings & northings    

   Numeric coordinates:
   • may be separated by a space, comma or both
   • pairs may contain different numbers of digits in Orkney & Shetland
      where northings extend beyond 999999 to require 7 digits, e.g.
      Ronas Hill (Shetland): 430530,1183500 (HU 30530 83500)
...
pub fn get_elevations(
   ...
   for i in 0..coords.len() {
      ...
      let grid_block_offset = 
         FILE_SIG.len() as i64 
         + (grid_blocks_to_jump * HEADER_BLOCK_LENGTH);     
      ...
      let data_placeholders_to_jump = 
         (n_addr_rows * DATA_COLS_IN_10_GRID) + e_addr_cols;

      // Can now determine the indidual data block address to jump to within the grid block
      let data_block_address_offset = (
         grid_block_offset 
         + GRID_IDENT_LEN 
         + (data_placeholders_to_jump * ADDRESS_LENGTH)) as u64 ;
      ...
      let data_rows_to_jump = 
         (data_rows * ELEVATIONS_PER_COL) 
         + data_cols;
...
fn get_infill_coords(coord_start: OSCoords, coord_end: OSCoords) -> Vec<OSCoords> {
   ...
   */
   ...
   let diagonal_diff = ((easting_diff * easting_diff) as f64 + (northing_diff * northing_diff) as f64).sqrt();
...
fn get_full_coord_pair(str_pair:Vec<String>) -> OSCoords {
   ...
   // Converts a vec of two number strings to five digit coordinates
